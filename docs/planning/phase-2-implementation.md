...
Rust reads `files.backup` and `files.backupCount` itself (P1 AD-8 pattern). `state.json` gains the Rust-owned `session` (`{ "paths": […], "active": 1 }`) and the webview-owned `ui.files` (with `machineId` per file); the P1 1 MiB cap stays (`ui.files` at its caps is about 200 KiB). New folders: `<config>/profiles/`, `<config>/codes/`, `<data>/recovery/<session>/`, `<data>/backups/<fnv(dir)>/<name>/`.
