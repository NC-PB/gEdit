...
- **M7 — never losing work.** Before every save that overwrites a file, the version on disk is copied aside — into gEdit's own data folder by default, several versions deep, so a DNC drip-feeder or a CAM folder watcher never finds a `.bak` to post to a machine; `.bak` beside the file and "keep none" are the other two settings. A copy that cannot be made is a question to the programmer ("Save without a backup?"), never a silent save, and a write that fails afterwards names the copy. **Crash recovery**: every document with unsaved changes is snapshotted off the keystroke path, every 30 s and whenever the window loses focus, into `<data>/recovery/<session>/`; a heartbeat file decides what is a leftover, so a second gEdit is never mistaken for a crashed one; an entry is dropped on save, on close and on a clean quit, and deliberately **not** on `RunEvent::Exit`, because a Windows logoff ends there too. Restoring writes nothing to disk and says per snapshot what it would do to the file. **Session restore and per-file memory** — the open files and the active tab, the cursor, the scroll position, the bookmarks, and a dialect or machine chosen by hand (an explicit "none" included) — each behind its own setting, with the recovery dialog deciding before the session reopens anything. And **read-only documents**: the file's own attribute, which gEdit never changes and which sends Save to Save As, kept apart from the lock a programmer puts on a tab to keep a hand off a proven program.

The milestones after it — Okuma OSP and Sinumerik turning (M8), compare and search, block skip, checks and extents (M9), multi-channel programs (M10), the code inspector and address arithmetic (M11), and templates, user profiles and the Phase 2 exit (M12) — are still ahead. §5 of the implementation plan is the running list.
